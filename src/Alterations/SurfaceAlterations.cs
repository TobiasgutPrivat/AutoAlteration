public class Surface(CustomSurfaceAlteration SurfaceAlt, string Surface, bool light = false,Alteration? sceneryAlteration = null) : Alteration {
    ...
    public override void Run(Inventory inventory, Map map) {
        if (light) {
            ...
            Inventory Platform = inventory.Select("Platform").Any(VanillaSurfaces); //temp
            Platform.Edit().RemoveKeyword(VanillaSurfaces).AddKeyword(Surface).Replace(inventory, map); //temp
